...
public class Sg_SweetHeart : SmallGame
{
    ...
    private List<int> spriteIndex = new List<int>();
    void Start()
    {
        //自动化游戏:1，将按钮禁用；2，将
        //如果是自动游戏的话
    ...
    IEnumerator SmallGameEffect()
    {
        if (currentValue < maxValue)
        {
            ...
            if (value.x > 0)
            {
                ...
                curTreasure.GetComponentInChildren<UISprite>().spriteName = "xin"+index;
            ...
            else
            {
                money_small = value.y;
                smallTotalWin += money_small;
                curTreasure.GetComponentInChildren<UISprite>().enabled = false;
